...
from bbq_core.objects_map.utils.structures import DetectionList
from bbq_core.objects_map.utils.objects import process_pcd, get_bounding_box
...
class DetectionsAssembler:
    ...
    def __call__(self, step_idx, color, depth, intrinsics, pose, masks_result, descriptors):
        ...
        segmentation_vis = color.copy() #.cpu().numpy()
    ...
    def create_object_pcd(self, mask, depth_array, cam_K):
        ...
        mask = np.logical_and(mask, np.logical_and(depth_array > 0, depth_array < 5.0))
